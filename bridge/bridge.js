 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.codex-whatsapp/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.codex-whatsapp', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.codex-whatsapp', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.codex-whatsapp', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.codex-whatsapp', 'audio_cache');
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Codex WhatsApp', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore the gateway's own reply messages in self-chat mode to avoid loops.
