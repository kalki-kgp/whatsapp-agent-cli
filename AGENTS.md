...
A two-process system that connects a WhatsApp number to a local coding CLI (`codex` or `Codex`). Each WhatsApp chat gets its own persistent session state so different chats can target different repos or models independently.
...
**Codex** (`run_claude`): calls `Codex -p --output-format json --permission-mode bypassPermissions --add-dir <root> [--resume | --session-id]`. Reply and `session_id` come from the JSON payload on stdout.
